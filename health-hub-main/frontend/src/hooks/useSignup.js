...
export const useSignup = () => {
  ...
  const signup = async (fName, surname, dob, doctor, email, password, phone) => {
    // Set loading state to true and clear any previous errors
    setIsLoading(true)
    setError(null)

    // Make a signup request to the backend
    const response = await fetch('https://healthhubbackendapi.onrender.com/api/user/signup', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ fName, surname, dob, doctor, email, password, phone })
    })
